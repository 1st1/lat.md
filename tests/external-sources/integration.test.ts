...
describe.sequential('external source commands', () => {
  ...
  beforeAll(async () => {
    ...
    project = createExternalProject(fixture, {
      ...
      body: 'See [[upstream:guide.md#Navigation]] for the contract.',
    });
    roots.push(project.root);
    writeFileSync(
      join(project.root, 'app.ts'),
      '// ' +
        '@lat: [[upstream:guide.md#Navigation]]\nexport const app = true;\n',
    );
    ctx = {
  ...
  // @lat: [[tests/external-tests#External Sources#Commands and MCP]]
  it('supports observational management and existing content commands', async () => {
    ...
    const section = await sectionCommand(ctx, 'upstream:guide.md#Navigation');
    ...
    const expanded = await expandCommand(
      ctx,
      'Explain [[upstream:guide.md#Navigation]]',
    );
    ...
    const add = await externalAddCommand(
      ...
      'added',
      fixture.repoUrl,
      { commit: 'v2', strategy: 'checkout', prefix: 'docs' },
    );
    expect(add.isError).not.toBe(true);
    expect(readFileSync(join(addProject.latDir, 'lat.md'), 'utf8')).toContain(
      'external-sources:',
    );
    expect(readFileSync(join(addProject.latDir, 'lat.md'), 'utf8')).toContain(
      fixture.commit2,
  ...
  it('exposes external metadata and content through MCP', async () => {
    ...
    try {
      ...
      const section = await client.callTool({
        name: 'lat_section',
        arguments: { query: 'upstream:guide.md#Navigation' },
      });
      expect(
