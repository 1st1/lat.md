...
describe('hybrid search', () => {
  ...
  it('archives legacy models and preserves existing backup files', async () => {
    ...
    await legacy.execute('CREATE TABLE meta(key TEXT PRIMARY KEY,value TEXT)');
    await legacy.execute(
      "INSERT INTO meta VALUES('embedding_model','local:test:2')",
    );
    legacy.close();
  ...
  it('publishes a replacement while another process reads the old FTS generation', async () => {
    const f = fixture('# Guide\n\nneedle original');
