...
`--profile` adds a nested timing report for every validator and its major operations. Markdown timing distinguishes file reads, hashing, persistent parser-cache hits or misses, cache publication, and actual parser work; a fully warm run reports no AST parsing. Repeated work is aggregated with call counts, average and maximum duration, and the slowest file or target so large-repository bottlenecks remain visible without one output line per file. Concurrent timings remain attributed to their initiating validator and may overlap within the total wall time.
