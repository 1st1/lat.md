...
Usage: `lat check [md|links|code-refs|index|sections] [--ignore-local-overrides] [-- <directory>]`
...
1. **Non-markdown files** — any file without a `.md` extension is flagged as an error (only markdown belongs in the checked directory), except for `config.local.json` at its root
...
Key resolution order: `LAT_LLM_KEY` > `LAT_LLM_KEY_FILE` > `LAT_LLM_KEY_HELPER` > config file `llm_key`. This applies to `lat search`, `lat reindex`, `lat init`, `lat check`, and the MCP `lat_search` tool.
