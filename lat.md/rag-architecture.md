...
Results sort by descending hybrid score. In this example, a section ranked tenth by both channels outranks one ranked first only lexically. The score is a ranking signal, not a confidence percentage; its maximum is $2/61 \approx 0.03279$. RRF ignores raw score margins and treats a channel's missing candidate as zero evidence, so candidate selection affects the final ordering. [[rag-architecture#Candidate retrieval and fusion]] details the thresholds, retrieval limits, and tie handling.
...
```mermaid
flowchart TB
    source["Analyzed Markdown"] --> chunks["Owned passages<br/>with bounded heading context"]
    chunks --> hash["Hash complete embedding input<br/>and model/tokenizer/chunk fingerprint"]
    hash --> cached{"Hash in active index?"}
    cached -->|Yes| reuse["Reuse vector"]
    cached -->|No| embed["Generate and validate vector"]
    reuse --> rows["Update changed sections and passage metadata"]
    embed --> rows
    rows --> prune["Remove unreferenced vectors"]
```
...
The RRF formula in [[rag-architecture#Retrieval pipeline]] combines these section ranks; an absent channel contributes zero. Final ties use section ID. Maximum-passage aggregation avoids dilution from unrelated passages, but longer sections still have more opportunities to produce a high-scoring passage.
...
```mermaid
flowchart TB
    old[("Published generation A")] -.-> readers["Existing readers keep A<br/>Windows readers use private copies"]
    lock["Acquire writer lock"] --> stage["Create staging generation B"]
    old -->|Copy for incremental update| stage
    stage --> update["Apply and validate changes"]
    update --> outcome{"Outcome"}
    outcome -->|Failed or unchanged| discard["Discard B<br/>Manifest still names A"]
    outcome -->|Changed successfully| checkpoint["Checkpoint and close B"]
    checkpoint --> publish["Atomically replace manifest<br/>to name B"]
    publish --> next["New readers open B"]
```
