...
import { z } from 'zod';
import { flattenSections, parseSections } from './lattice.js';
import {
  SOURCE_EXTENSIONS,
  parseSourceSymbols,
  type SourceSymbol,
...
export type ResolvedExternalContent = {
...
async function localCheckoutStatus(
  ...
  try {
    if (lstatSync(path).isSymbolicLink()) {
      throw new Error('checkout root cannot be a symlink');
    }
    if (!statSync(path).isDirectory()) throw new Error('not a directory');
    const realPath = realpathSync(path);
    const root = await git(['-C', path, 'rev-parse', '--show-toplevel']);
    if (resolve(root) !== realPath)
      throw new Error('path is not the checkout root');
    ...
    for (const remote of remotes) {
      const candidate = await git(['-C', path, 'remote', 'get-url', remote]);
      try {
        if (normalizeExternalRepoUrl(candidate) === source.repo) matches = true;
      } catch {
        // Unsupported local remote; another remote may identify the repository.
      }
    }
    if (!matches) throw new Error('no Git remote matches the configured repo');
...
export function parseExternalTarget(
  ...
  const ext = extname(authoredPath).toLowerCase();
  ...
  return {
    ...
    repositoryPath,
    fragment,
    identity: `${handle}:${authoredPath}${fragment ? `#${fragment}` : ''}`,
  };
}
    ...
    const request = httpsRequest(
      ...
      (response) => {
        ...
        if ([301, 302, 303, 307, 308].includes(status)) {
          ...
          void readHttps(next, redirects + 1, ca).then(resolveBuffer, reject);
          return;
        }
        if (status !== 200) {
          response.resume();
        ...
        response.on('end', () => resolveBuffer(Buffer.concat(chunks)));
...
async function selectFragment(
  ...
  Omit<
    ...
    'target' | 'source' | 'provider' | 'fullContent'
  >
> {
  const lines = fullContent.split('\n');
  if (target.authoredPath.toLowerCase().endsWith('.md')) {
    if (!target.fragment)
      return {
        content: fullContent,
        startLine: 1,
    ...
    const virtual = `/external/${target.handle}/${target.authoredPath}`;
    const sections = flattenSections(parseSections(virtual, fullContent, '/'));
    const wanted = target.fragment.toLowerCase();
    const match = sections.find(
      (section) =>
        section.id.split('#').slice(1).join('#').toLowerCase() === wanted ||
        section.githubSlug?.toLowerCase() === wanted ||
        section.heading.toLowerCase() === wanted,
    );
    if (!match)
      throw new Error(
        `Markdown heading "${target.fragment}" not found in "${target.authoredPath}"`,
      );
    const fullEndLine = (section: (typeof sections)[number]): number =>
      section.children.length === 0
        ? section.endLine
        : fullEndLine(section.children[section.children.length - 1]);
    const endLine = fullEndLine(match);
    return {
      content: lines.slice(match.startLine - 1, endLine).join('\n'),
  ...
  const symbols = await parseSourceSymbols(target.authoredPath, fullContent);
  const symbol = sourceSymbol(symbols, target.fragment);
  if (!symbol)
    throw new Error(
      `symbol "${target.fragment}" not found in "${target.authoredPath}"`,
    );
  return {
...
export async function addCanonicalExternalSource(
  ...
  const match = content.match(/^---\r?\n([\s\S]*?)\r?\n---(?:\r?\n|$)/);
  const document = parseDocument(match?.[1] ?? 'lat: {}\n');
