...
function externalSectionTarget(
  handle: string,
  authoredPath: string,
  section: Section,
): string {
  const fragment = section.id.split('#').slice(1).join('#');
  return `${handle}:${authoredPath}${fragment ? `#${fragment}` : ''}`;
}
...
export async function getViewExternal(
  ...
  if (resolved.kind === 'markdown') {
    const virtualPath = join(
      ...
      resolved.target.authoredPath,
    );
    const tree = parse(resolved.fullContent);
    ...
    const resolver = await createResolver(resolved.target.authoredPath);
    ...
    const baseTarget = `${resolved.target.handle}:${resolved.target.authoredPath}`;
    const targetHeadings = new Map<string, string>();
    const flat = flattenSections(sections);
    targetHeadings.set(baseTarget, flat[0]?.githubSlug ?? '');
    for (const section of flat) {
      targetHeadings.set(
        externalSectionTarget(
          resolved.target.handle,
          resolved.target.authoredPath,
          section,
        ),
      ...
      const headingTarget = `${baseTarget}#${section.heading}`;
      if (!targetHeadings.has(headingTarget)) {
        targetHeadings.set(headingTarget, section.githubSlug ?? '');
      }
      if (section.githubSlug) {
        targetHeadings.set(
          `${baseTarget}#${section.githubSlug}`,
          section.githubSlug,
        );
      }
    }
    return {
      ...
      document: {
        ...
        gitHtml: null,
        graphNodeIds: {},
        tableOfContents: buildViewTableOfContents(sections, tree, {
          errors: [],
          gitTree: null,
        }),
        errors: [],
